...
public class PatientModel {
	...
	public long add(PatientBean bean) throws Exception {
		...
		pstmt.setDate(3, new java.sql.Date(bean.getDov().getTime()));
	...
	public void update(PatientBean bean) throws Exception {
		...
		PreparedStatement pstmt = conn.prepareStatement(
				"update st_patient set name = ?, dateOfVisit = ?, mobile = ?, decease = ? where id = ?");

		pstmt.setString(1, bean.getName());
		pstmt.setDate(2, new java.sql.Date(bean.getDov().getTime()));
	...
	public List search(PatientBean bean, int pageNo, int pageSize) throws Exception {
		...
		if (bean != null) {
			...
			if (bean.getDov() != null && bean.getDov().getTime() > 0) {
				Date d = new Date(bean.getDov().getTime());
